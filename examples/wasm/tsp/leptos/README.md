# orx-parallel wasm TSP leptos

This example shows the recommended web structure for `orx-parallel` with a Leptos UI hosted by Vite:

- `computation/` contains pure Rust TSP logic
- `wasm_bindings/` exposes a thin wasm API for that computation
- `components/` contains the Leptos UI and calls into the wasm bindings
- `app/` is the browser app that builds, serves, and boots the generated wasm package

The same structure works for other parallelizable Rust workloads too. The important part is the separation: keep the algorithm in Rust, keep the wasm layer thin, keep the Leptos UI focused on orchestration and presentation, and keep the browser app responsible for hosting and bootstrapping.
...
This crate contains the Leptos UI. It renders the page, owns UI state, and calls into JavaScript for the worker lifecycle.
...
- expose the TypeScript worker bridge used by the Leptos UI
...
3. The Leptos UI in `components/` creates or loads a TSP instance.
...
7. If the request is parallel, the worker calls `init_parallel_runtime(thread_count)` before the first `run_search`.
8. `run_search` executes the Rust computation and returns the result to the worker.
9. The worker posts the result back to the bridge, which resolves the promise back to the Leptos UI.
...
3. Build the UI around a Rust UI layer plus a browser host.
   ...
   In this example, the Leptos UI calls a small TypeScript bridge that owns the worker lifecycle. That keeps browser-specific bootstrapping in `app/` while keeping the UI itself in Rust.
...
9. Test each layer independently.
   ...
   Verify the Rust computation with ordinary Rust tests, verify the wasm boundary in `wasm_bindings/`, verify the Leptos UI behavior in `components/`, and verify the browser integration in `app/`. This is the main advantage of the four-project layout.
