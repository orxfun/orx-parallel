...
- `init_wasm_parallel_runtime(num_threads)`: initializes the shared thread pool for parallel execution.
...
Call `init_wasm_parallel_runtime` once before the first `run_search` call in each worker.

If the target does not support atomics and shared memory, `init_wasm_parallel_runtime` cannot succeed.
