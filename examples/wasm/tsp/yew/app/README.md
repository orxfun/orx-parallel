# Vite host app for the Yew `orx-parallel` demo

This demo uses **Vite** with **TypeScript** to host a wasm-built Yew UI.
...
- call `start_app()` to mount the Yew UI
- the worker also imports `init_wasm_parallel_runtime(...)` and `run_search(...)` from the same generated package
...
- import `./search-runner.ts` so the Yew UI can call the worker bridge through `globalThis.runSearchAlgorithm`
- define `globalThis.highlightCodeBlocks` so the Yew UI can ask the host page to highlight code blocks
...
- import `init`, `init_wasm_parallel_runtime`, and `run_search` from `./pkg/components.js`
