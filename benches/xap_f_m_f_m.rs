...
fn iter<E: Exp>(inputs: &[u64]) -> E::Out {
    let iter = inputs.iter().copied().filter(f2).map(f1).filter(f3).map(f4);
    E::out(iter)
}

fn xap<E: Exp>(inputs: &[u64]) -> E::Out {
    let xap = Id::new().filter(f2).map(f1).filter(f3).map(f4);
...
fn run(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("xap_f_m_f_m");
