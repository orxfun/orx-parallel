...
fn run_rayon(input: &[u64], heavy: bool, nt: usize, list: bool) -> Output {
    ...
    match (heavy, list) {
        (true, false) => {
            Output::Vec(pool.install(|| input.into_par_iter().map(h_m).filter(f).collect()))
        }
        (false, false) => {
            Output::Vec(pool.install(|| input.into_par_iter().map(l_m).filter(f).collect()))
        }
        (true, true) => Output::VecList(
            pool.install(|| input.into_par_iter().map(h_m).filter(f).collect_vec_list()),
        ),
        (false, true) => Output::VecList(
            pool.install(|| input.into_par_iter().map(l_m).filter(f).collect_vec_list()),
        ),
    }
}
    ...
    match heavy {
        true => {
            ...
            match (fixed, list) {
                ...
                (true, false) => {
                    Output::Vec(par.runner(Runner::fixed(Pool::default(nt))).collect())
                }
...
fn run(c: &mut Criterion) {
    ...
    Exp.bench(c, "collect_mf", &treatments, &variants);
}
