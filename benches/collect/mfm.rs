...
fn run_rayon(input: &[u64], heavy: bool, nt: usize, list: bool) -> Output {
    ...
    match (heavy, list) {
        (true, false) => {
            Output::Vec(pool.install(|| input.into_par_iter().map(m).filter(f).map(h_m2).collect()))
        }
        (false, false) => {
            Output::Vec(pool.install(|| input.into_par_iter().map(m).filter(f).map(l_m2).collect()))
        }
        (true, true) => Output::VecList(pool.install(|| {
            input
                .into_par_iter()
                .map(m)
                .filter(f)
        ...
        (false, true) => Output::VecList(pool.install(|| {
            input
                .into_par_iter()
                .map(m)
                .filter(f)
...
fn run(c: &mut Criterion) {
    ...
    Exp.bench(c, "collect_mfm", &treatments, &variants);
}
