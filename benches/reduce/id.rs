    ...
    fn execute(
        ...
        match alg_variant {
            ...
            Method::Rayon { nt } => {
                ...
                pool.install(|| {
                    Some(match h {
                        true => input.into_par_iter().copied().reduce(|| 0, h_r),
                        false => input.into_par_iter().copied().reduce(|| 0, l_r),
                    })
                })
            }
            Method::Orx { nt } => match h {
                true => input.into_par().num_threads(*nt).copied().reduce(h_r),
                false => input.into_par().num_threads(*nt).copied().reduce(l_r),
            },
...
fn run(c: &mut Criterion) {
    ...
    Exp.bench(c, "reduce_id", &treatments, &variants);
}
