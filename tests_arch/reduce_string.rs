...
pub fn min<R: Par<Item = String>>(par: R, len: usize) {
    let expected = some_if(len > 0, 0.to_string());
    let result = par.min();
    assert_eq!(result, expected);
}

pub fn max<R: Par<Item = String>>(par: R, len: usize) {
...
pub fn min_by<R: Par<Item = String>>(par: R, len: usize) {
...
pub fn max_by<R: Par<Item = String>>(par: R, len: usize) {
    #[allow(clippy::ptr_arg)]
    fn compare(a: &String, b: &String) -> Ordering {
        let a: i64 = a.parse().expect("ok");
        let b: i64 = b.parse().expect("ok");
...
pub fn min_by_key<R: Par<Item = String>>(par: R, len: usize) {
    #[allow(clippy::ptr_arg)]
...
pub fn max_by_key<R: Par<Item = String>>(par: R, len: usize) {
    #[allow(clippy::ptr_arg)]
    fn get_key(a: &String) -> usize {
        let a: i64 = a.parse().expect("ok");
        match a % 2 {
