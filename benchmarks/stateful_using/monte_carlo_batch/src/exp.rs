...
impl ThreadState {
    fn new() -> Self {
        Self { rng: ChaCha8Rng::seed_from_u64(0), trace: Vec::new() }
    }
}
...
fn inputs(len: usize) -> Vec<u64> {
    ...
    (0..len).map(|idx| rng.random::<u64>() ^ (idx as u64).rotate_left(17)).collect()
}
...
fn simulate(state: &mut ThreadState, sample_seed: u64, steps: usize, with_trace: bool) -> Stats {
    ...
    Stats { sum_final: pos as i64, min_seen, trace_digest: digest, samples: 1 }
}

fn seq_stats(input: &[u64], with_trace: bool, steps: usize) -> Option<Stats> {
    let mut state = ThreadState::new();
    input.iter().map(|seed| simulate(&mut state, *seed, steps, with_trace)).reduce(merge)
}

fn rayon_stats(input: &[u64], with_trace: bool, steps: usize) -> Option<Stats> {
    input.par_iter()
        .map_init(ThreadState::new, |state, seed| simulate(state, *seed, steps, with_trace))
        .reduce_with(merge)
}
...
impl Experiment for Exp {
    ...
    fn execute(&mut self, input_variant: &Self::InputFactors, alg_variant: &Self::AlgFactors, input: &Self::Input) -> Self::Output {
        ...
        match alg_variant {
            ...
            Method::OrxOnce | Method::OrxBasic | Method::OrxRayon => orx_stats(input, with_trace, steps),
        }
    }

    fn expected_output(&self, input_variant: &Self::InputFactors, input: &Self::Input) -> Option<Self::Output> {
        Some(seq_stats(input, input_variant.with_trace, input_variant.steps()))
    }
}
