...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Projected { user_bucket: u16, score: u64 }
...
fn parse_project_light(record: &LogRecord) -> Projected {
    ...
    Projected { user_bucket: (record.user_id % 1024) as u16, score: parsed.wrapping_add(record.payload_len as u64) }
}

fn parse_project_heavy(record: &LogRecord) -> Projected {
    let rounds = 8 + record.payload_len as usize / 96;
    let parsed = cpu_mix(rounds, record.payload_seed ^ record.ts ^ ((record.code as u64) << 21));
    Projected { user_bucket: (record.user_id % 1024) as u16, score: parsed.wrapping_add(record.payload_len as u64) }
}

fn keep(record: &LogRecord) -> bool { (record.severity >= 3 && !record.code.is_multiple_of(5)) || record.code == 777 }

fn collect_seq(input: &[LogRecord], heavy: bool) -> Vec<Projected> {
    input.iter().filter(|record| keep(record)).map(|record| if heavy { parse_project_heavy(record) } else { parse_project_light(record) }).collect()
}

fn collect_rayon(input: &[LogRecord], heavy: bool) -> Vec<Projected> {
    input.par_iter().filter(|record| keep(record)).map(|record| if heavy { parse_project_heavy(record) } else { parse_project_light(record) }).collect()
}

fn collect_orx(input: &[LogRecord], heavy: bool) -> Vec<Projected> {
    input.par().filter(|record| keep(record)).map(|record| if heavy { parse_project_heavy(record) } else { parse_project_light(record) }).collect()
}
...
impl Experiment for Exp {
    ...
    fn execute(&mut self, input_variant: &Self::InputFactors, alg_variant: &Self::AlgFactors, input: &Self::Input) -> Self::Output {
        match alg_variant { Method::Seq => collect_seq(input, input_variant.heavy), Method::Rayon => collect_rayon(input, input_variant.heavy), Method::OrxOnce | Method::OrxBasic | Method::OrxRayon => collect_orx(input, input_variant.heavy) }
    }

    fn expected_output(&self, input_variant: &Self::InputFactors, input: &Self::Input) -> Option<Self::Output> { Some(collect_seq(input, input_variant.heavy)) }
}
