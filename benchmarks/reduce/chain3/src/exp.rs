...
fn run_seq(input: &[u64], heavy: bool) -> u64 {
    match heavy {
        true => input.iter().filter(f).map(h_m).flat_map(fl).reduce(h_r),
        false => input.iter().filter(f).map(l_m).flat_map(fl).reduce(l_r),
    }
    .unwrap()
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> u64 {
    ...
    match heavy {
        true => par.filter(f).map(h_m).flat_map(fl).reduce(h_r),
        false => par.filter(f).map(l_m).flat_map(fl).reduce(l_r),
    }
    .unwrap()
}
