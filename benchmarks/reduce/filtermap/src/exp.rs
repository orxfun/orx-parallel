...
fn run_seq(input: &[u64], heavy: bool) -> u64 {
    match heavy {
        true => input.iter().filter_map(f).reduce(h_r),
        false => input.iter().filter_map(f).reduce(l_r),
    }
    .unwrap()
}
...
fn run_rayon(input: &[u64], heavy: bool) -> u64 {
    use rayon::prelude::*;
    match heavy {
        true => input.into_par_iter().filter_map(f).reduce_with(h_r),
        false => input.into_par_iter().filter_map(f).reduce_with(l_r),
    }
    .unwrap()
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> u64 {
    ...
    match heavy {
        true => par.filter_map(f).reduce(h_r),
        false => par.filter_map(f).reduce(l_r),
    }
    .unwrap()
}
