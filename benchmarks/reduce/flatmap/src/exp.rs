...
fn run_seq(input: &[u64], heavy: bool) -> u64 {
    match heavy {
        true => input.iter().flat_map(h_m).reduce(h_r),
        false => input.iter().flat_map(l_m).reduce(l_r),
    }
    .unwrap()
}
...
fn run_rayon(input: &[u64], heavy: bool) -> u64 {
    use rayon::prelude::*;
    match heavy {
        true => input.into_par_iter().flat_map_iter(h_m).reduce_with(h_r),
        false => input.into_par_iter().flat_map_iter(l_m).reduce_with(l_r),
    }
    .unwrap()
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> u64 {
    ...
    match heavy {
        true => par.flat_map(h_m).reduce(h_r),
        false => par.flat_map(l_m).reduce(l_r),
    }
    .unwrap()
}
