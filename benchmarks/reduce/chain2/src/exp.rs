...
fn run_seq(input: &[u64], heavy: bool) -> u64 {
    match heavy {
        true => input.iter().filter(f).map(h_m).reduce(h_r),
        false => input.iter().filter(f).map(l_m).reduce(l_r),
    }
    .unwrap()
}
...
fn run_rayon(input: &[u64], heavy: bool) -> u64 {
    use rayon::prelude::*;
    match heavy {
        true => input.into_par_iter().filter(f).map(h_m).reduce_with(h_r),
        false => input.into_par_iter().filter(f).map(l_m).reduce_with(l_r),
    }
    .unwrap()
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> u64 {
    ...
    match heavy {
        true => par.filter(f).map(h_m).reduce(h_r),
        false => par.filter(f).map(l_m).reduce(l_r),
    }
    .unwrap()
}
