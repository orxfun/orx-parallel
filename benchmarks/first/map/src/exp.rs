...
fn run_seq(input: &[u64], heavy: bool) -> u64 {
    match heavy {
        true => input.iter().map(h_m).find(find),
        false => input.iter().map(l_m).find(find),
    }
    .unwrap()
}
...
fn run_rayon(input: &[u64], heavy: bool) -> u64 {
    use rayon::prelude::*;
    match heavy {
        true => input.into_par_iter().map(h_m).find_first(find),
        false => input.into_par_iter().map(l_m).find_first(find),
    }
    .unwrap()
}
...
fn run_orx(input: &[u64], heavy: bool, ord: IterationOrder) -> u64 {
    ...
    match heavy {
        true => par.map(h_m).find(find),
        false => par.map(l_m).find(find),
    }
    .unwrap()
}
