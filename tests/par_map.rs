...
#[test]
#[should_panic]
fn par_map_panics_when_zero_num_threads() {
    let vec = vec![1, 4, 2, 1, 5, 6];
    let _ = vec.into_par().map(|x| x.to_string()).num_threads(0);
}

#[test]
#[should_panic]
fn par_map_panics_when_zero_chunk_size() {
    let vec = vec![1, 4, 2, 1, 5, 6];
    let _ = vec.into_par().map(|x| x.to_string()).chunk_size(0);
}
...
fn par_map_next() {
    fn test(num_threads: usize, chunk_size: usize) {
        ...
        let par = par.map(|x| x * 3);
        let par = par.num_threads(num_threads).chunk_size(chunk_size);
        assert_eq!(par.next(), Some((0, 13 * 3)));
    }
    test_different_params(test);
...
fn par_map_find() {
    fn test(num_threads: usize, chunk_size: usize) {
        ...
        let par = par.map(|x| x * 3);
        let par = par.num_threads(num_threads).chunk_size(chunk_size);
        assert_eq!(par.find(|x| x >= &40), Some((14 - 13, 42)));
    }
    test_different_params(test);
