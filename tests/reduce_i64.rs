...
pub fn reduce<R: Reduce<i64>>(par: R, len: usize) {
...
pub fn sum<R: Reduce<i64>>(par: R, len: usize) {
    let expected = (0..len).sum::<usize>() as i64;
    let result = par.sum();
    assert_eq!(result, expected);
}

pub fn min<R: Reduce<i64>>(par: R, len: usize) {
    let expected = some_if(len > 0, 0);
    let result = par.min();
    assert_eq!(result, expected);
}

pub fn max<R: Reduce<i64>>(par: R, len: usize) {
    let expected = some_if(len > 0, len as i64 - 1);
    let result = par.max();
    assert_eq!(result, expected);
}

pub fn min_by<R: Reduce<i64>>(par: R, len: usize) {
...
pub fn max_by<R: Reduce<i64>>(par: R, len: usize) {
    fn compare(a: &i64, b: &i64) -> Ordering {
        match b % 2 {
            0 => a.cmp(b),
            _ => Ordering::Greater,
...
pub fn min_by_key<R: Reduce<i64>>(par: R, len: usize) {
...
pub fn max_by_key<R: Reduce<i64>>(par: R, len: usize) {
    fn get_key(a: &i64) -> usize {
        match a % 2 {
            0 => 0,
            _ => *a as usize,
