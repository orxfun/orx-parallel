        ...
        self.pool_mut().scoped_computation(move |s| {
            while let Some(th_idx) = Self::do_spawn_new(spawned, state) {
                ...
                <Self::Pool as ParThreadPool>::run_in_scope(&s, move || {
                    let value = th::next::<Self, _, _, _, _, _, _>(th_idx, state, iter, x);
                    results.push(value);
                });
            }
        });
        ...
        E: Send,
    {
        ...
        self.pool_mut().scoped_computation(move |s| {
            while let Some(th_idx) = Self::do_spawn_new(spawned, state) {
                spawned += 1;
                <Self::Pool as ParThreadPool>::run_in_scope(&s, move || {
                    let value = th::next_any::<Self, _, _, _, _, _, _>(th_idx, state, iter, x);
                    results.push(value);
                });
            }
        });
        ...
        E: Send,
    {
        ...
        self.pool_mut().scoped_computation(move |s| {
            while let Some(th_idx) = Self::do_spawn_new(spawned, state) {
                spawned += 1;
                <Self::Pool as ParThreadPool>::run_in_scope(&s, move || {
                    let value = th::reduce::<Self, _, _, _, _, _, _, _>(th_idx, state, iter, x, f);
                    results.push(value);
                });
            }
        });
