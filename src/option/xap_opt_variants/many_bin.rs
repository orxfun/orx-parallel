...
pub struct XapOptManyBin<M, X1, X2>
where
...
impl<M, X1, X2> Clone for XapOptManyBin<M, X1, X2>
where
    X1: Xap<O = Option<M>, Size = Many>,
    X2: Xap<I = M, Size = Bin>,
{
...
impl<M, X1, X2> Copy for XapOptManyBin<M, X1, X2>
where
    X1: Xap<O = Option<M>, Size = Many>,
    X2: Xap<I = M, Size = Bin>,
{
...
unsafe impl<M, X1, X2> Send for XapOptManyBin<M, X1, X2>
where
    X1: Xap<O = Option<M>, Size = Many>,
    X2: Xap<I = M, Size = Bin>,
{
...
impl<M, X1, X2> XapOptManyBin<M, X1, X2>
where
    X1: Xap<O = Option<M>, Size = Many>,
    X2: Xap<I = M, Size = Bin>,
{
...
impl<M, X1, X2> XapOpt for XapOptManyBin<M, X1, X2>
where
    X1: Xap<O = Option<M>, Size = Many>,
    X2: Xap<I = M, Size = Bin>,
{
    ...
    type Results = IterOptManyBin<M, <<X1 as Xap>::Values as IntoIterator>::IntoIter, X2>;

    fn xap_res(&self, i: Self::I) -> Self::Results {
        let iter = self.x1.xap(i).into_iter();
        IterOptManyBin { iter, x2: self.x2 }
    }
    ...
    where
        H: Fn(Self::O) -> Q + Copy + Send,
    {
        XapOptManyBin::new(self.x1, self.x2.map(h))
    }

    type Inspect<H>
        = XapOptManyBin<M, X1, X2::Inspect<H>>
    where
    ...
    where
        H: Fn(&Self::O) + Copy + Send,
    {
        XapOptManyBin::new(self.x1, self.x2.inspect(h))
    }
    ...
    type Mapped<H>
        = XapOptManyBin<M, X1, X2::Mapped<H>>
    where
        H: Map<I = Self::O>;
    ...
    where
        H: Map<I = Self::O>,
    {
        XapOptManyBin::new(self.x1, self.x2.mapped(h))
    }
}
