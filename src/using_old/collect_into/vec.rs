...
impl<O> UParCollectIntoCoreOld<O> for Vec<O>
where
    O: Send + Sync,
{
        ...
        M1: Fn(&mut U::Item, I::Item) -> O + Sync,
    {
        match m.iter().try_get_len() {
            None => {
                ...
                let split_vec = split_vec.u_m_collect_into_old::<R, _, _, _>(m);
    ...
    {
        let split_vec = SplitVec::with_doubling_growth_and_max_concurrent_capacity();
        let split_vec = split_vec.u_x_collect_into_old::<R, _, _, _, _>(x);
        extend_vec_from_split(self, split_vec)
    }
}
