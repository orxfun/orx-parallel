use crate::{
    IterationOrder, Params, runner::DefaultRunner, using_old::UsingClone,
    using_old::computations::UM,
};
...
fn u_m_map_collect(n: usize, nt: usize, chunk: usize, ordering: IterationOrder) {
    let offset = 33;
    ...
    let mut u = 0;
    for i in 0..offset {
        let value = map(&mut u, i.to_string());
        output.push(value.clone());
        expected.push(value);
    }
    expected.extend(input.clone().into_iter().map(|x| map(&mut u, x)));
