...
use crate::{ChunkSize, IterationOrder, NumThreads, ParIterFallible, Sum};
use crate::{
...
pub trait ParIterOptionalDeprecated<R = DefaultRunner>
where
    R: ParallelRunner,
{
    type Success;

    type RegularItem: IntoOption<Self::Success>;
    ...
    fn map<Out, Map>(self, map: Map) -> impl ParIterOptionalDeprecated<R, Success = Out>
    where
        Self: Sized,
        ...
        Out: Send,
    {
        ...
        let map = par.map(move |x| x.into_option().map(map.clone()));
        map.into_optional()
    }
    ...
    // {
    //     let par = self.into_regular_par();
    //     let par = par.map(|x| x.into_result_with_unit_err());
    //     let par = par.into_fallible();
    //     let par = par.filter(filter);
    //     let par = par.into_regular_par();
    //     // let par = self.into_regular_par();
    //     // let filter_map = par.filter_map(move |x| match x.into_option() {
    //     //     Ok(x) => match filter(&x) {
    //     //         true => Some(Ok(x)),
    //     //         false => None,
    //     //     },
    ...
    // {
    //     let par = self.into_regular_par();
    //     let map = par.flat_map(move |x| match x.into_option() {
