...
impl SizePair for BinMany {
    ...
    type XapUseOptResult<M, X1, X2>
        = IterUseOptBinMany<<X2::Values as IntoIterator>::IntoIter>
    where
        ...
        X2: XapUse<U = X1::U, I = M, Size = Self::S2>,
    {
        match x1.bin_value(u, i) {
            Some(Some(a)) => IterUseOptBinMany::success(Some(x2.xap_use(u, a).into_iter())),
            Some(None) => IterUseOptBinMany::fail(),
            None => IterUseOptBinMany::success(None),
        }
    }
    ...
    type XapUseResResult<M, E, X1, X2>
        = IterUseResBinMany<<X2::Values as IntoIterator>::IntoIter, E>
    where
    ...
    {
        match x1.bin_value(u, i) {
            Some(Ok(a)) => IterUseResBinMany::success(Some(x2.xap_use(u, a).into_iter())),
            Some(Err(e)) => IterUseResBinMany::fail(e),
            None => IterUseResBinMany::success(None),
        }
    }
}
...
pub enum IterUseOptBinMany<I: Iterator> {
    Success(Option<I>),
    Fail(bool),
}

impl<I: Iterator> IterUseOptBinMany<I> {
    pub fn success(i: Option<I>) -> Self {
        Self::Success(i)
    }
...
impl<I: Iterator> Iterator for IterUseOptBinMany<I> {
    type Item = Option<I::Item>;
...
impl<I: FusedIterator> FusedIterator for IterUseOptBinMany<I> {}

impl<I: ExactSizeIterator> ExactSizeIterator for IterUseOptBinMany<I> {
    fn len(&self) -> usize {
        match self {
            Self::Success(Some(i)) => i.len(),
            Self::Success(None) => 0,
...
pub enum IterUseResBinMany<I: Iterator, E> {
    Success(Option<I>),
    Fail(Option<E>),
}

impl<I: Iterator, E> IterUseResBinMany<I, E> {
    pub fn success(i: Option<I>) -> Self {
        Self::Success(i)
    }
...
impl<I: Iterator, E> Iterator for IterUseResBinMany<I, E> {
    type Item = Result<I::Item, E>;
...
impl<I: FusedIterator, E> FusedIterator for IterUseResBinMany<I, E> {}

impl<I: ExactSizeIterator, E> ExactSizeIterator for IterUseResBinMany<I, E> {
    fn len(&self) -> usize {
        match self {
            Self::Success(Some(i)) => i.len(),
            Self::Success(None) => 0,
