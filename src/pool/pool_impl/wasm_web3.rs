...
#[wasm_bindgen(module = "/src/pool/pool_impl/worker_helpers3.js")]
extern "C" {
...
/// Initializes the worker-backed wasm thread runtime for `WasmWebPool3`.
...
pub fn init_thread_pool(num_threads: usize) -> js_sys::Promise {
    ...
    match WASM_WEB3_THREAD_POOL_STATE.compare_exchange(
        ...
        Err(_) => unreachable!("invalid wasm-web-threads3 init state"),
    }
}

#[cfg(target_feature = "atomics")]
/// Returns `(configured_threads, spawned_workers)` for the active wasm-web-threads3 runtime.
pub fn wasm_web3_runtime_info() -> (usize, usize) {
    let configured_threads = WASM_WEB3_THREAD_POOL_NUM_THREADS.load(Ordering::SeqCst);
...
#[cfg(target_feature = "atomics")]
#[wasm_bindgen]
/// Worker entrypoint called from `worker_helpers3.js` after wasm init.
pub fn wasm_web3_start_worker() {
...
fn assert_wasm_thread_pool_initialized() {
    ...
    assert_eq!(
        ...
        WASM_WEB3_THREAD_POOL_INITIALIZED,
        "Wasm web3 thread pool is not initialized. Call and await init_thread_pool(...) before running parallel computations."
    )
}
...
fn runtime() -> &'static Arc<Inner> {
    ...
    WASM_WEB3_RUNTIME.get_or_init(|| {
        ...
        let num_threads = NonZeroUsize::new(num_threads)
            .expect("wasm web3 configured thread count must be > 0 after init_thread_pool");
...
pub struct WasmWebPool3 {
    max_num_threads: NonZeroUsize,
}

impl Default for WasmWebPool3 {
    fn default() -> Self {
...
impl ParThreadPool for WasmWebPool3 {
...
impl ParThreadPool for &WasmWebPool3 {
    ...
    where
        ...
        W: Fn() + Send + 'scope + 'env,
    {
        <WasmWebPool3 as ParThreadPool>::run_in_scope(s, work)
    }
