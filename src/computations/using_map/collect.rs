use super::m::UsingM;
#[cfg(test)]
use crate::IterationOrder;
use crate::runner::{ComputationKind, ParallelRunner, ParallelRunnerCompute};
use orx_concurrent_iter::ConcurrentIter;
use orx_pinned_vec::IntoConcurrentPinnedVec;

impl<U, I, O, M1> UsingM<U, I, O, M1>
where
    U: Clone + Send,
    I: ConcurrentIter,
    O: Send + Sync,
    M1: Fn(&mut U, I::Item) -> O + Send + Sync,
{
    pub fn collect_into<R, P>(self, pinned_vec: P) -> (usize, P)
    where
        R: ParallelRunner,
        P: IntoConcurrentPinnedVec<O>,
    {
        let p = self.params();
        match (p.is_sequential(), p.iteration_order) {
            (true, _) => (0, self.sequential(pinned_vec)),
            #[cfg(test)]
            (false, IterationOrder::Arbitrary) => {
                let runner = self.runner::<R>();
                runner.using_m_collect_in_arbitrary_order(self, pinned_vec)
            }
            (false, _) => {
                let runner = self.runner::<R>();
                runner.using_m_collect_ordered(self, pinned_vec)
            }
        }
    }

    fn sequential<P>(self, mut pinned_vec: P) -> P
    where
        P: IntoConcurrentPinnedVec<O>,
    {
        let (_, mut using, iter, map1) = self.destruct();

        let iter = iter.into_seq_iter();
        for i in iter {
            pinned_vec.push(map1(&mut using, i));
        }

        pinned_vec
    }

    fn runner<R: ParallelRunner>(&self) -> R {
        R::new(
            ComputationKind::Collect,
            self.params(),
            self.iter().try_get_len(),
        )
    }
}
