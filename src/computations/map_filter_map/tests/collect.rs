...
fn xyz_mf(n: usize, nt: usize, chunk: usize, ordering: CollectOrdering, actual_filter: bool) {
    let offset = 33;
...
fn abc() {
    ...
    let nt = 2;
    let chunk = 1;
    let ordering = CollectOrdering::Arbitrary;

    let offset = 33;

    let input: Vec<_> = (0..n).map(|x| x.to_string()).collect();
    let map1 = |x: String| Atom(format!("{}!", x));
    let filter = |x: &String| !x.starts_with('1');
