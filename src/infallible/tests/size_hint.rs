...
fn size_hint_one_for_exact_sized_input() {
    ...
    let par = input.into_par();
    assert_eq!(par.size_hint(), (4, Some(4)));

    let par = par.map(|x| x + 1);
    assert_eq!(par.size_hint(), (4, Some(4)));
}
...
fn size_hint_bin_for_exact_sized_input() {
    ...
    let par = input.into_par().filter(|x| *x < 2);
    assert_eq!(par.size_hint(), (0, Some(4)));

    let par = par.filter_map(|x| (x < 3).then_some(6 + x));
    assert_eq!(par.size_hint(), (0, Some(4)));
}
...
fn size_hint_bin_for_unknown_sized_input() {
    ...
    let par = input.iter_into_par().filter(|x| *x < 2);
    assert_eq!(par.size_hint(), (0, None));
...
fn size_hint_many_for_exact_sized_input() {
    ...
    let par = input.into_par().flat_map(|x| (0..2).map(move |y| x + y));
    assert_eq!(par.size_hint(), (0, None));

    let par = par.flat_map(|x| (0..2).map(move |y| x + y));
    assert_eq!(par.size_hint(), (0, None));
}
